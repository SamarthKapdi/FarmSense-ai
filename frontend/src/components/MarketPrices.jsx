...
export default function MarketPrices() {
    ...
    const fetchTrend = async () => {
        ...
        try {
            const response = await fetch(
                `${BASE_URL}/market/trend?crop=${encodeURIComponent(selectedCrop)}&state=${encodeURIComponent(selectedState)}&days=30`,
                { headers: { Authorization: `Bearer ${token}` } }
            )
            const json = await response.json()
            setTrend(json.data || [])
    ...
    return (
        <div className="px-4 pt-6 pb-4 max-w-2xl mx-auto fade-in">
                ...
                <div className="flex gap-1 bg-[var(--bg-elevated)] rounded-lg p-0.5 border border-[var(--border)]">
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-colors ${
                            activeView === 'table' ? 'bg-emerald-600 text-white' : 'text-[var(--text-secondary)]'
                        }`}
                    >Table</button>
                    <button
                        onClick={() => setActiveView('chart')}
                        className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-colors ${
                            activeView === 'chart' ? 'bg-emerald-600 text-white' : 'text-[var(--text-secondary)]'
                        }`}
                    >Trend</button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
                ...
                >
                    {STATES.map(state => (
                        <option key={state} value={state} className="bg-[var(--bg-card)]">{state}</option>
                    ))}
                </select>
            </div>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-[var(--border)] bg-[var(--bg-elevated)]">
                                <th className="text-left text-[var(--text-muted)] text-xs py-3 px-4 font-semibold">Market</th>
                                <th className="text-right text-[var(--text-muted)] text-xs py-3 px-3 font-semibold">Min</th>
                                <th className="text-right text-[var(--text-muted)] text-xs py-3 px-3 font-semibold">Max</th>
                                <th className="text-right text-[var(--text-muted)] text-xs py-3 px-4 font-semibold">Modal</th>
                            </tr>
                        </thead>
                        <tbody>
                            {prices.map((p, i) => (
                                <tr key={i} className="border-b border-[var(--border)] last:border-b-0 hover:bg-[var(--hover-bg)] transition-colors">
                                    <td className="py-3 px-4 text-[var(--text-primary)] font-medium">{p.market}</td>
                                    <td className="py-3 px-3 text-right text-red-400 text-xs font-medium">{p.minPrice}</td>
                                    <td className="py-3 px-3 text-right text-emerald-500 text-xs font-medium">{p.maxPrice}</td>
                                    <td className="py-3 px-4 text-right text-[var(--text-primary)] font-bold">{p.modalPrice}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                    <div className="px-4 py-2 border-t border-[var(--border)]">
                        <p className="text-[var(--text-muted)] text-xs">Updated: {prices[0]?.arrivalDate || 'Today'} • Prices in ₹/quintal</p>
                    </div>
                </div>
            )}
            ...
            {!loading && activeView === 'chart' && (
                <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-4">
                    ...
                    <p className="text-xs text-[var(--text-muted)] mb-4">{selectedState}</p>
                    {trendLoading ? (
                        ...
                        <p className="text-center text-[var(--text-muted)] py-12">No trend data available</p>
                    )}
                </div>
            )}
            ...
            {!loading && prices.length === 0 && !error && (
                <div className="flex flex-col items-center py-12 text-center">
                    ...
                    <p className="text-[var(--text-muted)] text-sm">Select a crop and state to view prices</p>
                </div>
            )}
        </div>
    )
