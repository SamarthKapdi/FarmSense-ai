...
export default function AnalyticsCharts() {
    ...
    const fetchAll = async () => {
      ...
      try {
        ...
        const [d1, d2, d3, d4] = await Promise.all([
          fetch(apiUrl('/analytics/disease-breakdown'), { headers }).then((r) =>
            r.json()
          ),
          fetch(apiUrl('/analytics/monthly-trends'), { headers }).then((r) =>
            r.json()
          ),
          fetch(apiUrl('/analytics/crop-distribution'), { headers }).then((r) =>
            r.json()
          ),
          fetch(apiUrl('/analytics/severity-summary'), { headers }).then((r) =>
            r.json()
          ),
        ])
  ...
  const severityPieData = severityData
    ? [
        ...
        {
          name: 'Moderate',
          value: severityData.moderate,
          color: SEVERITY_COLORS.moderate,
        },
        {
          name: 'Severe',
          value: severityData.severe,
