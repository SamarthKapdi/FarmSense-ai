...
export default function ImageUploader({ language, farmerId, token, onResult }) {
    ...
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        canvas.toBlob(
          ...
          'image/jpeg',
          0.6 // Balanced compression for faster uploads
        )
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await new Promise((resolve, reject) => {
        ...
        xhr.onerror = () =>
          reject(new Error('AI service unavailable. Please retry in a moment.'))
        xhr.ontimeout = () =>
          reject(
            new Error(
              'AI analysis is taking too long. Please try a smaller image or try again.'
            )
          )
  ...
  return (
    <div className="px-4 pt-6 pb-4 max-w-lg mx-auto fade-in">
      ...
      >
        {selectedImages.length > 0 ? (
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-gray-300 font-medium">{t('upload.tap')}</p>
            <p className="text-gray-500 text-xs">{t('upload.format')}</p>
          </div>
        )}
        <input
          ref={fileInputRef}
      ...
      {error && (
        <div
          className="mt-4 bg-red-950/50 border border-red-800/50 
