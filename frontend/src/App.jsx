...
import { getUnreadNotifications, markAllNotificationsRead } from "./services/notificationApi";
...
function AppContent() {
    ...
    const [activeTab, setActiveTab] = useState(user?.role === "ROLE_ADMIN" ? "admin" : "detect");
    ...
    useEffect(() => {
        ...
        const streamUrl = user?.role === 'ROLE_ADMIN' ? `${baseUrl}/sse/broadcast` : `${baseUrl}/sse/stream`;
    ...
    return (
        <div className="min-h-screen bg-[var(--bg-main)] pb-20 transition-colors duration-300">
                ...
                <div className="flex items-center justify-between max-w-6xl mx-auto">
                    ...
                    <div className="flex items-center gap-2">
                            ...
                            {showNotifications && (
                                <div className="absolute right-0 top-full mt-2 w-80 bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl shadow-lg overflow-hidden z-50">
                                    <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border)]">
                                        ...
                                        {unreadCount > 0 && (
                                            <button onClick={handleMarkAllRead} className="text-xs text-emerald-500 hover:text-emerald-400 font-medium">
                                    ...
                                    <div className="max-h-72 overflow-y-auto">
                                        {notifications.length === 0 ? (
                                            <div className="px-4 py-8 text-center">
                                                ...
                                                <p className="text-[var(--text-muted)] text-sm">{t('notifications.no_notifications')}</p>
                                            </div>
                                        ) : (
                                            notifications.slice(0, 8).map(n => (
                                                <div key={n.id} className="px-4 py-3 border-b border-[var(--border)] hover:bg-[var(--hover-bg)] transition-colors cursor-pointer">
                                                    <div className="flex items-start gap-2">
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            <p className="text-[var(--text-primary)] text-xs font-medium truncate">{n.title}</p>
                                                            {n.message && <p className="text-[var(--text-muted)] text-xs mt-0.5 truncate">{n.message}</p>}
                                                            <p className="text-[var(--text-muted)] text-[10px] mt-1">{timeAgo(n.createdAt)}</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            ))
                        ...
                        <select value={language} onChange={(e) => setLanguage(e.target.value)}
                            ...
                            {LANGUAGES.map(l => (
                                <option key={l.code} value={l.code} className="bg-[var(--bg-card)]">
                                    {l.code.toUpperCase()}
                                </option>
                            ))}
                        </select>
            ...
            {(showNotifications || showMore) && (
                <div className="fixed inset-0 z-30" onClick={() => { setShowNotifications(false); setShowMore(false); }} />
            )}
            ...
            <main>
                {activeTab === "detect" && !showResults && (
                    <ErrorBoundary name="Disease Detection">
                        <ImageUploader language={language} farmerId={user?.userId} token={token} onResult={handleResult} onLanguageChange={setLanguage} />
                    </ErrorBoundary>
                )}
                ...
                {activeTab === "detect" && showResults && currentResult && (
                    <ErrorBoundary name="Results Dashboard">
                        {Array.isArray(currentResult) ? (
                            <BatchResultsDashboard results={currentResult} language={language} farmerId={user?.userId} token={token} onBack={handleBackToUpload} />
                        ) : (
                            <ResultsDashboard result={currentResult} language={language} farmerId={user?.userId} token={token} onBack={handleBackToUpload} />
                        )}
                    </ErrorBoundary>
                )}
                ...
                {activeTab === "chat" && (
                    <ErrorBoundary name="KrishiGPT Chat">
                        <KrishiGPTChat language={language} farmerId={user?.userId} token={token} onLanguageChange={setLanguage} />
                    </ErrorBoundary>
                )}

                {activeTab === "weather" && <ErrorBoundary name="Weather"><WeatherWidget /></ErrorBoundary>}
                    ...
                    {primaryTabs.map(tab => {
                        ...
                        return (
                            <button key={tab.id} onClick={() => handleTabClick(tab.id)}
                                ...
                                {isActive && <div className="absolute top-0 left-1/2 -translate-x-1/2 w-6 h-0.5 bg-emerald-500 rounded-full" />}
                                <span className={`text-lg transition-transform ${isActive ? 'scale-110' : ''}`}>{tab.icon}</span>
                                <span>{tab.label}</span>
                            </button>
                        );
                    })}
