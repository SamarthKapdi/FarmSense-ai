const BASE_URL = process.env.REACT_APP_API_BASE_URL ? `${process.env.REACT_APP_API_BASE_URL}/agronomist` : 'http://localhost:8080/api/agronomist';
...
export const getDiseaseTrends = async () => {
    ...
    if (!response.ok) throw new Error(data.message || "Failed to fetch disease trends");
    return data.data;
};
...
export const getPendingVerifications = async () => {
    ...
    const data = await response.json();
    if (!response.ok) throw new Error(data.message || "Failed to fetch pending verifications");
    return data.data;
};
...
export const verifyDiagnosis = async (reportId, correctDisease, notes) => {
    ...
    const data = await response.json();
    if (!response.ok) throw new Error(data.message || "Failed to verify diagnosis");
    return data.data;
};
...
export const publishAdvisory = async (advisory) => {
    ...
    const data = await response.json();
    if (!response.ok) throw new Error(data.message || "Failed to publish advisory");
    return data.data;
};
...
export const getAdvisories = async () => {
    ...
    const data = await response.json();
    if (!response.ok) throw new Error(data.message || "Failed to fetch advisories");
    return data.data;
};
