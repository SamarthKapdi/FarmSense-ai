const BASE_URL = process.env.REACT_APP_API_BASE_URL ? `${process.env.REACT_APP_API_BASE_URL}/weather` : 'http://localhost:8080/api/weather';
...
export const getDailyForecast = async (lat, lon, days = 10) => {
    const response = await fetch(`${BASE_URL}/daily?lat=${lat}&lon=${lon}&days=${days}`);
...
export const searchCity = async (name) => {
    ...
    const response = await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(name)}&count=10&language=en&format=json`);
...
export const reverseGeocode = async (lat, lon) => {
    ...
    try {
        ...
        return {
            name: data.address.city || data.address.town || data.address.village || data.address.state || "Current Location",
