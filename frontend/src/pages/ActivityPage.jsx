...
export default function ActivityPage() {
  ...
  const filteredScans = safeScans.filter((s) => {
    if (
      scanFilter &&
      !s.diseaseName?.toLowerCase().includes(scanFilter.toLowerCase())
    )
      return false
  ...
  // Analytics helpers
  const diseaseBreakdown = safeScans.reduce((acc, s) => {
  ...
  const cropBreakdown = safeScans.reduce((acc, s) => {
    const name = s.cropName || 'Unknown'
    acc[name] = (acc[name] || 0) + 1
    return acc
  }, {})
  const topCrops = Object.entries(cropBreakdown)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5)
  const maxCropCount = topCrops.length > 0 ? topCrops[0][1] : 1

  const langBreakdown = safeChats.reduce((acc, c) => {
    ...
    <div className="px-4 pt-5 pb-4 max-w-lg mx-auto fade-in">
        ...
        <div className="space-y-4 fade-in">
          ...
          <div className="disease-card bg-gradient-to-br from-primary/30 to-darker">
            ...
            <p className="text-gray-400 text-xs mt-1">
              Member since{' '}
              {stats?.joinedDate
                ? new Date(stats.joinedDate).toLocaleDateString('en-IN', {
                    month: 'long',
                    year: 'numeric',
                  })
                : 'recently'}
            ...
            <p className="text-gray-500 text-xs mt-0.5">
              Last active:{' '}
              {stats?.lastActive ? timeAgo(stats.lastActive) : 'Just now'}
            </p>
          </div>
          ...
          <div className="disease-card">
            <h4 className="text-[var(--text-primary)] font-bold mb-3 text-sm">
              ⚡ Recent Activity
            </h4>
            {safeActivities.length === 0 ? (
              ...
              <div className="space-y-3">
                {safeActivities.slice(0, 8).map((a, i) => (
          ...
          {filteredScans.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 text-sm">
                No scans yet! Upload your first crop photo
              </p>
            </div>
          ) : (
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className={getSeverityClass(scan.severity)}>
                        {scan.severity}
                      </span>
                    </div>
                  </div>
        ...
        <div className="space-y-3 fade-in">
          ...
          {safeChats.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 text-sm">
                No chats yet! Ask KrishiGPT anything
              </p>
            </div>
          ) : (
            safeChats.map((chat, index) => (
              <div key={chat.id || index} className="disease-card">
                <button
                  onClick={() =>
                    setExpandedChat(expandedChat === index ? null : index)
                  }
                  className="w-full text-left"
                >
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-gray-500 text-xs">
                      {chat.language
                        ? LANG_NAMES[chat.language] || chat.language
                        : ''}{' '}
      ...
      {activeTab === 'analytics' && (
        <div className="space-y-5 fade-in">
          ...
          {safeScans.length === 0 && safeChats.length === 0 ? (
            <div className="text-center py-12">
              <div className="text-5xl mb-3">📊</div>
              <p className="text-gray-400 text-sm">
                Use the app more to see analytics!
              </p>
            </div>
          ) : (
            <>
              ...
              {topDiseases.length > 0 && (
                <div className="disease-card">
                  <h4 className="text-[var(--text-primary)] font-bold text-sm mb-3">
                    🦠 Disease Breakdown
                  </h4>
