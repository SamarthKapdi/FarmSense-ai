...
const CROPS = ["Tomato", "Rice", "Wheat", "Corn", "Potato", "Cotton", "Sugarcane", "Soybean", "Onion", "Other"];
const LANGUAGES = [
...
export default function ProfilePage({ token }) {
    ...
    const handleChangePassword = async () => {
        ...
        if (!/[A-Z]/.test(newPassword)) { setPwError("Must contain 1 uppercase"); return; }
        if (!/[0-9]/.test(newPassword)) { setPwError("Must contain 1 number"); return; }
    ...
    return (
        <div className="p-4 max-w-lg mx-auto space-y-4 fade-in">
            {/* Profile Header */}
            <div className="disease-card" style={{ padding: '20px', textAlign: 'center' }}>
                ...
                <p className="text-gray-600 text-xs mt-1">
                    Member since {profile?.memberSince ? new Date(profile.memberSince).toLocaleDateString() : '—'}
                </p>
                {stats && (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', marginTop: '16px' }}>
                        <div style={{ background: 'rgba(82,183,136,0.05)', borderRadius: '10px', padding: '10px' }}>
                            <div style={{ color: 'var(--accent)', fontSize: '18px', fontWeight: 800 }}>{stats.totalScans || 0}</div>
                            <div style={{ color: 'var(--text-secondary)', fontSize: '10px' }}>Scans</div>
                        </div>
                        <div style={{ background: 'rgba(82,183,136,0.05)', borderRadius: '10px', padding: '10px' }}>
                            <div style={{ color: 'var(--accent)', fontSize: '18px', fontWeight: 800 }}>{stats.totalChats || 0}</div>
                            <div style={{ color: 'var(--text-secondary)', fontSize: '10px' }}>Chats</div>
                        </div>
                        <div style={{ background: 'rgba(82,183,136,0.05)', borderRadius: '10px', padding: '10px' }}>
                            <div style={{ color: 'var(--accent)', fontSize: '18px', fontWeight: 800 }}>{stats.diseasesDetected || 0}</div>
                            <div style={{ color: 'var(--text-secondary)', fontSize: '10px' }}>Diseases</div>
                        </div>
                    </div>
                )}
                {deferredPrompt && (
            ...
            {/* Edit Profile */}
            <div className="disease-card" style={{ padding: '20px' }}>
                <h3 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: '14px', marginBottom: '16px' }}>
                    ✏️ Edit Profile
                </h3>
                <div className="space-y-3">
                    ...
                    <div>
                        ...
                        <select value={preferredCrop} onChange={e => setPreferredCrop(e.target.value)} className={inputClass}>
            ...
            <div className="disease-card" style={{ padding: '20px' }}>
                ...
                <div className="space-y-3">
                    ...
                    <input type="password" value={newPassword} onChange={e => setNewPassword(e.target.value)}
                        placeholder="New Password (min 8, 1 uppercase, 1 number)" className={inputClass} />
