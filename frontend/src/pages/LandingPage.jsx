...
const STEPS = [
  { num: '01', icon: '📸', title: 'Capture', desc: 'Take a clear photo of the affected crop leaf using your phone camera' },
  { num: '02', icon: '🧠', title: 'AI Analysis', desc: 'Our vision model processes the image and identifies the disease instantly' },
  { num: '03', icon: '💊', title: 'Get Treatment', desc: 'Receive instant treatment plans — organic, chemical, and preventive measures' },
]
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-main)', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: 0, pointerEvents: 'none' }}>
        ...
        <div style={{
          ...
          borderRadius: '50%', background: 'radial-gradient(circle, rgba(5,150,105,0.1) 0%, transparent 70%)',
          filter: 'blur(60px)',
        }} />
        {/* Bottom-left orb */}
        <div style={{
          position: 'absolute', bottom: '-20%', left: '-10%', width: '550px', height: '550px',
          borderRadius: '50%', background: 'radial-gradient(circle, rgba(52,211,153,0.07) 0%, transparent 70%)',
          filter: 'blur(60px)',
        }} />
      </div>
      ...
      {/* ═══════════════════════════════════════════════════════ */}
      <section style={{
        position: 'relative', zIndex: 10, padding: '100px 24px 80px',
        ...
        <div style={{
          ...
          <span className="leaf-pulse" style={{ fontSize: '16px' }}>🌾</span>
          Powered by Spring AI + Ollama Vision
        </div>

        <h1 style={{
          fontSize: 'clamp(36px, 6vw, 64px)', fontWeight: 900,
          lineHeight: 1.08, marginBottom: '24px', letterSpacing: '-1.5px',
        }}>
        ...
        <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', flexWrap: 'wrap', marginBottom: '56px' }}>
          <button onClick={() => onNavigate('register')} className="btn-primary" style={{ padding: '18px 40px', fontSize: '17px' }}>
            Start Detecting Free 🚀
          </button>
          <button onClick={() => document.getElementById('features')?.scrollIntoView({ behavior: 'smooth' })}
            className="btn-ghost" style={{ padding: '18px 40px', fontSize: '17px' }}>
            See How It Works ↓
          </button>
        </div>
        ...
        <div style={{
          ...
          {STATS.map((s) => (
            <div key={s.label} className="card" style={{ padding: '18px 8px', textAlign: 'center', cursor: 'default' }}>
              <div style={{ fontSize: '22px', marginBottom: '6px' }}>{s.icon}</div>
              <div style={{ color: 'var(--accent)', fontSize: '24px', fontWeight: 800 }}>{s.value}</div>
              <div style={{ color: 'var(--text-muted)', fontSize: '11px', marginTop: '2px', fontWeight: 500 }}>
                {s.label}
              </div>
            </div>
          ))}
      ...
      <section id="features" style={{
        position: 'relative', zIndex: 10, padding: '80px 24px',
        ...
        <div className="stagger-in" style={{
          ...
          {FEATURES.map((f, i) => (
            <div key={i} className="card" style={{ padding: '32px', position: 'relative', overflow: 'hidden', cursor: 'default' }}>
              {/* Decorative circle */}
              <div style={{
              ...
              <div style={{
                display: 'inline-flex', padding: '10px', borderRadius: 'var(--radius-md)',
                background: 'var(--accent-muted)', marginBottom: '16px', fontSize: '28px',
              }}>
                {f.icon}
              </div>
              <div style={{
                display: 'inline-block', marginLeft: '12px', padding: '3px 10px', borderRadius: 'var(--radius-full)',
                background: 'var(--accent-muted)', fontSize: '11px', fontWeight: 600, color: 'var(--accent)',
              }}>
                {f.tag}
              </div>
              <h3 style={{
                color: 'var(--text-primary)', fontWeight: 700, fontSize: '17px',
            ...
            <div key={i} style={{ display: 'flex', gap: '20px', alignItems: 'stretch' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: '56px' }}>
                <div className="btn-primary" style={{
                  ...
                  flexShrink: 0, padding: 0, fontSize: '15px', boxShadow: 'var(--shadow-glow)',
                }}>
                  {step.num}
                </div>
                {i < STEPS.length - 1 && (
      ...
      <section style={{
        ...
        <div className="card" style={{
          textAlign: 'center', padding: '44px 36px', minHeight: '220px',
          ...
          <p style={{
            color: 'var(--text-primary)', fontSize: '17px', lineHeight: 1.8,
            fontStyle: 'italic', marginBottom: '20px', fontWeight: 400,
          }}>
            "{TESTIMONIALS[activeTestimonial].text}"
          </p>
          <div>
            <p style={{ color: 'var(--accent)', fontWeight: 700, fontSize: '15px' }}>
              {TESTIMONIALS[activeTestimonial].name}
            </p>
            <p style={{ color: 'var(--text-muted)', fontSize: '13px', marginTop: '2px' }}>
              {TESTIMONIALS[activeTestimonial].location} • {TESTIMONIALS[activeTestimonial].crop} Farmer
            </p>
          </div>
      ...
      <section style={{
        ...
        <div style={{
          background: 'linear-gradient(145deg, var(--accent-muted), var(--bg-card))',
          border: '1px solid var(--border)',
          borderRadius: 'var(--radius-2xl)', padding: '56px 36px',
          ...
          <p style={{
            color: 'var(--text-secondary)', fontSize: '16px', marginBottom: '36px', lineHeight: 1.7,
          }}>
      ...
      <footer style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
          {['Spring Boot', 'React', 'Spring AI', 'Ollama', 'PostgreSQL'].map((tech) => (
            <span key={tech} style={{
              color: 'var(--text-muted)', fontSize: '12px', fontWeight: 500,
              background: 'var(--accent-muted)', padding: '5px 14px',
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '11px', marginTop: '6px', opacity: 0.6 }}>
          By{' '}
          <a href="https://github.com/SamarthKapdi" style={{ color: 'var(--accent)', textDecoration: 'none' }}>
            Samarth Kapdi
          </a>
        </p>
      </footer>
