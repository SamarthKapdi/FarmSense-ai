  ...
  const services = [
    {
      ...
      desc: 'Core REST gateway • Port 8080',
      status: 'UP',
      icon: '☕',
    },
    {
      name: 'PostgreSQL',
      desc: 'Primary database • Port 5432',
      status: health?.database || 'UNKNOWN',
      icon: '🗄️',
    },
    {
      name: 'Gemini Vision',
      desc: 'Disease detection • Hosted API',
      status: health?.gemini || 'UNKNOWN',
      icon: '🧠',
    },
    {
      name: 'Groq Chat',
      desc: 'KrishiGPT advisory • Hosted API',
      status: health?.groq || 'UNKNOWN',
      icon: '💬',
    },
  ]
    ...
    return (
      <div className="p-8 max-w-7xl mx-auto space-y-6">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="h-24 bg-[var(--bg-card)] rounded-2xl animate-pulse border border-[var(--border)]"
          ></div>
        ))}
      </div>
    )
    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-[var(--text-secondary)] text-sm mt-1">
            Real-time health of all microservices and resources.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {services.map((svc, i) => (
          <div
            key={i}
            className="bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl p-5 flex items-center justify-between hover:border-emerald-500/20 transition-colors"
          >
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl p-6">
        ...
        <div className="space-y-5">
          ...
          <ResourceBar
            label="Hosted AI Availability"
            value={health?.gemini === 'UP' && health?.groq === 'UP' ? 78 : 0}
            color={
              health?.gemini === 'UP' && health?.groq === 'UP' ? 'amber' : 'red'
            }
          />
...
function StatusPill({ status }) {
  ...
  return (
    ...
    >
      <span
        className={`w-1.5 h-1.5 rounded-full ${isUp ? 'bg-emerald-500 animate-pulse' : 'bg-red-500'}`}
      ></span>
      {status}
    </div>
  )
