...
export default function AIMonitoringPage() {
    ...
    const requestTypeData = [
        ...
        { name: 'Treatment Plan', value: Math.floor((stats?.scansToday || 2) * 0.6) },
    ...
    return (
        <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-8">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-[var(--text-primary)]">AI Monitoring</h2>
                    <p className="text-[var(--text-secondary)] text-sm mt-1">Inference engine telemetry and performance metrics.</p>
                </div>
                <div className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-sm font-semibold ${
                    health?.ollama === 'UP' ? 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20' : 'bg-red-500/10 text-red-500 border-red-500/20'
                }`}>
                    <span className={`w-2 h-2 rounded-full ${health?.ollama === 'UP' ? 'bg-emerald-500 animate-pulse' : 'bg-red-500'}`}></span>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
                <MetricCard label="AI Prompts Today" value={stats?.chatsToday || 0} icon="🧠" />
                <MetricCard label="Disease Scans" value={stats?.scansToday || 0} icon="📷" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl p-6">
                    <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-4">Response Latency Trend</h3>
                    <ResponsiveContainer width="100%" height={240}>
                        <LineChart data={latencyData}>
                            ...
                            />
                            <Line type="monotone" dataKey="latency" stroke="#3b82f6" strokeWidth={2} dot={{ fill: '#3b82f6', r: 3 }} />
                        </LineChart>
                    </ResponsiveContainer>
                </div>
                ...
                <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl p-6">
                    <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-4">Request Distribution</h3>
                    <ResponsiveContainer width="100%" height={200}>
                        <PieChart>
                            <Pie data={requestTypeData} cx="50%" cy="50%" innerRadius={45} outerRadius={75} paddingAngle={4} dataKey="value">
            ...
            {/* Model Info */}
            <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl p-6">
                <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-4">Active Models</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <ModelCard name="Llama 3" purpose="KrishiGPT Chat & Advisory" status={health?.ollama} />
                    <ModelCard name="Llama 3.2 Vision" purpose="Crop Image Analysis" status={health?.ollama} />
                </div>
            </div>
        </div>
    );
...
function MetricCard({ label, value, icon, accent, good }) {
    return (
        <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl p-5">
            ...
            <p className={`text-2xl font-bold ${accent ? 'text-blue-500' : good ? 'text-emerald-500' : 'text-[var(--text-primary)]'}`}>{value}</p>
        </div>
    );
}
    ...
    return (
        <div className="flex items-center justify-between p-4 bg-[var(--hover-bg)] rounded-xl border border-[var(--border)]">
            <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-emerald-500/10 rounded-lg flex items-center justify-center text-lg border border-emerald-500/20">🧠</div>
                <div>
                    <p className="font-semibold text-sm text-[var(--text-primary)]">{name}</p>
                    <p className="text-xs text-[var(--text-muted)]">{purpose}</p>
                </div>
            </div>
            <span className={`text-xs font-bold px-2 py-1 rounded-lg ${isUp ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}>
                {isUp ? 'Online' : 'Offline'}
            </span>
        </div>
    );
