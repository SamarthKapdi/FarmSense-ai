...
export default function OverviewPage() {
  ...
  const cropPieData = (
    stats?.topCrops || ['Tomato', 'Wheat', 'Rice', 'Potato']
  ).map((crop, i) => ({
    name: crop,
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-[var(--text-secondary)] text-sm mt-1">
            Real-time telemetry from the FarmSense intelligence network.
          </p>
        </div>
        <div className="flex items-center gap-3">
          <ServiceBadge label="Database" status={health?.database} />
          <ServiceBadge label="Gemini" status={health?.gemini} />
          <ServiceBadge label="Groq" status={health?.groq} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <p className="font-bold text-[var(--text-primary)]">
                {stats?.mostCommonDisease || 'None Detected'}
              </p>
        ...
        {/* Activity Feed */}
        <div
          className="lg:col-span-2 bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl p-6 flex flex-col"
...
function ServiceBadge({ label, status }) {
  ...
  return (
    ...
    >
      <span
        className={`w-1.5 h-1.5 rounded-full ${isUp ? 'bg-emerald-500' : 'bg-red-500'}`}
      ></span>
      {label}
    </div>
  )
